...
def main(argv=None):
    ...
    if options.individual:  # Create a collection of configs
        ...
        if options.individual:
            file_list.extend(
                filter(
                    isVerilog,
                    map(
                        lambda file: abspath(options.individual) + "/" + file,
                        os.listdir(options.individual),
                    ),
                )
            )
        ...
        options.directory or options.files and len(args) == 3
    ):  # Create a single project based on a directory or file list
        base = args.pop()
        path = abspath(args.pop()) + "/"
        ...
        if options.directory:
            file_list.extend(
                filter(
                    isVerilog,
                    map(
                        lambda file: abspath(options.directory) + "/" + file,
...
def create_odin_projects(options, file_list, base, path):
    ...
    config = odin.config(
        verilog_files=create_common_verilog(file_list), debug_outputs=create_common_debug(options)
    )
    if soft:
...
def configOptParse():
    ...
    input = OptionGroup(
        parser,
        "Input Options",
        "These options specify what input options are available. At least one of these must be specified.",
    )
    input.add_option(
        "-a",
    ...
    output = OptionGroup(
        parser,
        "Output Options",
        "These options specify the different output options available, if more than one is specified multiple configs will be created.",
    )
    output.add_option(
        "-o",
        help="The blif output path to be used in the configuration file. Defaults to ./",
        action="store",
        dest="output",
        metavar="OUTPUT_PATH",
    )
    ...
    output.add_option(
        "--both",
        help="Generate a blif that uses both hard block multiliers and memories.",
        action="store_true",
    )
    output.add_option(
        "--mult", help="Generate a blif that uses hard block multiliers.", action="store_true"
    )
    output.add_option(
        "--mem", help="Generate a blif that uses hard block memories.", action="store_true"
    )
    ...
    mult = OptionGroup(
        parser,
        "Hard Block Multiplier Options",
        "Configure how ODIN will use hard block multipliers.",
    )
    mult.add_option(
        "--size", help="Specify the minimum size of a multiplier.", action="store", dest="size"
    )
    mult.add_option(
        "--fixed", help="Pad any extra pins on the multiplier.", action="store_true", dest="fixed"
    )
    mult.add_option(
        "--no_fixed",
        help="Remove any extra pins on the multiplier.",
        action="store_false",
        dest="fixed",
    )
    mult.add_option(
        "--fracture",
        help="split the multiplier if neccesary.",
        action="store_true",
        dest="fracture",
    )
    mult.add_option(
    ...
    mem = OptionGroup(
        parser, "Hard Block Memory Options", "Configure how ODIN will use hard block memories."
    )
    ...
    debug.add_option(
        "--no_ast_graph", help="Do not output an AST graph", action="store_false", dest="ast_graph"
    )
    debug.add_option(
        "--no_netlist_graph",
        help="Do not output a netlist graph",
        action="store_false",
        dest="netlist_graph",
    )
