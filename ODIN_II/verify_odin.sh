...
function populate_arg_from_file() {
		...
		do
			...
			then
				case _${_key} in
					...
					_circuits_dir)
						warn_is_defined "${_circuits_dir}" "${_key}"
					...
					;;_archs_dir)
						warn_is_defined "${_archs_dir}" "${_key}"
						_archs_dir="${_value}"
	...
	_circuits_dir=$(echo "${THIS_DIR}/${_circuits_dir}")
	_archs_dir=$(echo "${THIS_DIR}/${_archs_dir}")
	...
	if [ "_${_circuits_dir}" != "_" ]
	then
		_circuits_dir=$(readlink -f ${_circuits_dir})
		if [ ! -d "${_circuits_dir}" ]
		then
			echo "Passed an invalid directory for your circuit files ${_circuits_dir}"
			_exit_with_code "-1"
		fi
	fi
	...
	do
		circuit_relative_path="${_circuits_dir}/${circuit_list_items}"
	...
	if [ "_${_archs_dir}" != "_" ]
	then
		_archs_dir=$(readlink -f ${_archs_dir})
		if [ ! -d "${_archs_dir}" ]
		then
	...
	do
		arch_relative_path="${_archs_dir}/${arch_list_items}"
