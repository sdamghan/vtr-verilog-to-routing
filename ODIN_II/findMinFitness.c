...
int GS_ARRAY[GS_ITER]={64};
// 2,4,8,16,32,64};
int GC_ARRAY[GC_ITER]={8192};
...
void copy_stat(struct stats* copy_stat, struct stats* paste_stat) {
    ...
    paste_stat->avg_depth         = copy_stat->avg_depth;
...
void scan_stat (struct stats* curr_stat) {
    ...
    fscanf(fp, "%d,%f,%d,%d,%d,ADD,%d,%d,%d,%d,%d", 
                ...
                &curr_stat->avg_depth, &curr_stat->max_fanout, /*curr_stat->op,*/ &curr_stat->op_number, 
                &curr_stat->node_count, &curr_stat->RCA_number, &curr_stat->CSLA_number, &curr_stat->BE_CSLA_number);
}

void print_header () {
    printf("%s\n", "benchmark,mutation_rate,generation_size,generation_count,generation_number,fitness,critical_path,avg_depth,max_fanout,ADD,op_number,node_count,RCA_number,CSLA_number,BE_CSLA_number");
}
...
void print_stat (struct stats* temp) {
    ...
    printf("%d,%f,%d,%d,%d,%s,%d,%d,%d,%d,%d\n", 
            temp->generation_number, temp->fitness, temp->critical_path, temp->avg_depth,
